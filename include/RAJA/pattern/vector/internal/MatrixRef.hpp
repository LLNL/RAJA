...
//      }
      ...
      RAJA_HOST_DEVICE
      RAJA_INLINE
      MatrixProductRef<matrix_type, MatrixImpl<VECTOR_TYPE, LAYOUT, IDX_ROW, IDX_COL> >
      operator*(MatrixImpl<VECTOR_TYPE, LAYOUT, IDX_ROW, IDX_COL> const &x) const
      {
        return MatrixProductRef<matrix_type, MatrixImpl<VECTOR_TYPE, LAYOUT, IDX_ROW, IDX_COL> >(load(), x);
      }
