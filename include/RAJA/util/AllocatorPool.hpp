...
struct MemoryArena
{
  ...
  bool give(void* ptr, MemoryChunk& mem)
  {
 ...
 *   }
 *
 *   // returns the Platform of the allocator
...
struct AllocatorPool : Allocator
{
  ...
  AllocatorPool(allocator_type const& aloc = allocator_type{},
                size_t default_arena_size = default_default_arena_size)
    : m_default_arena_size(default_arena_size)
    , m_alloc(aloc)
    , m_name(std::string("RAJA::AllocatorPool<")+m_alloc.getName()+">")
  {
  }

  virtual ~AllocatorPool()
  {
    // With static objects like AllocatorPool, cudaErrorCudartUnloading is a possible
    // error with cudaFree
