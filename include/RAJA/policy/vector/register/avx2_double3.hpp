...
#ifndef RAJA_policy_vector_register_avx2_double3_HPP
#define RAJA_policy_vector_register_avx2_double3_HPP
...
namespace RAJA
{
  ...
  template<>
  class Register<vector_avx2_register, double, 3>:
    public internal::RegisterBase<Register<vector_avx2_register, double, 3>>
  {
    public:
      using self_type = Register<vector_avx2_register, double, 3>;
    ...
    public:
      ...
      void load(element_type const *ptr, size_t stride = 1){
        if(stride == 1){
          m_value = _mm256_maskload_pd(ptr, _mm256_set_epi64x(0, -1, -1, -1));
        }
        else{
      ...
      void store(element_type *ptr, size_t stride = 1) const{
        if(stride == 1){
          _mm256_maskstore_pd(ptr, _mm256_set_epi64x(0, -1, -1, -1), m_value);
        }
        else{
          ptr[0] = m_value[0];
