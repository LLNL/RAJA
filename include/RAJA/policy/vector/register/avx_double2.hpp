...
namespace RAJA
{
  ...
  class Register<vector_avx_register, double, 2> :
    public internal::RegisterBase<Register<vector_avx_register, double, 2>>
  {
    public:
    ...
    public:
      ...
      self_type add(self_type const &b) const {
        return self_type(_mm_add_pd(m_value, b.m_value));
      }

      RAJA_HOST_DEVICE
      RAJA_INLINE
      self_type subtract(self_type const &b) const {
        return self_type(_mm_sub_pd(m_value, b.m_value));
      }

      RAJA_HOST_DEVICE
      RAJA_INLINE
      self_type multiply(self_type const &b) const {
        return self_type(_mm_mul_pd(m_value, b.m_value));
      }
