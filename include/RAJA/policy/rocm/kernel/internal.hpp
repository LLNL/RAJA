...
#ifndef RAJA_policy_rocm_kernel_internal_HPP
#define RAJA_policy_rocm_kernel_internal_HPP
...
struct ROCmIndexCalc_Policy;


template <camp::idx_t ArgumentId>
struct ROCmIndexCalc_Policy<ArgumentId, seq_exec> {
  int i;
...
template <camp::idx_t ArgumentId>
struct ROCmIndexCalc_Policy<ArgumentId, rocm_thread_exec> {
...
struct ROCmIndexCalc;


template <typename SegmentTuple,
          typename... IndexPolicies,
          camp::idx_t... RangeInts>
struct ROCmIndexCalc<SegmentTuple,
 ...
 */
template <typename SegmentTuple>
struct ROCmIndexCalc<SegmentTuple, camp::list<>, camp::idx_seq<>> {
...
template <typename SegmentTuple>
using ROCmIndexCalc_Terminator =
    ROCmIndexCalc<SegmentTuple, camp::list<>, camp::idx_seq<>>;


template <typename IndexCalcBase, typename CalcN>
struct ROCmIndexCalc_Extender;
...
template <typename SegmentTuple,
          ...
          camp::idx_t... RangeInts,
          typename CalcN>
struct ROCmIndexCalc_Extender<ROCmIndexCalc<SegmentTuple,
                                            camp::list<CalcPolicies...>,
                                            camp::idx_seq<RangeInts...>>,
                              CalcN> {
  using type = ROCmIndexCalc<SegmentTuple,
                             camp::list<CalcPolicies..., CalcN>,
                             camp::idx_seq<RangeInts..., sizeof...(RangeInts)>>;
};

template <typename IndexCalcBase, typename CalcN>
using ExtendROCmIndexCalc =
    typename ROCmIndexCalc_Extender<IndexCalcBase, CalcN>::type;


template <camp::idx_t cur_stmt, camp::idx_t num_stmts>
struct ROCmStatementListExecutorHelper {

  using next_helper_t =
      ROCmStatementListExecutorHelper<cur_stmt + 1, num_stmts>;
...
struct ROCmStatementListExecutorHelper<num_stmts, num_stmts> {
...
struct ROCmStatementExecutor;

template <typename Data, typename StmtList, typename IndexCalc>
struct ROCmStatementListExecutor;


template <typename Data, typename... Stmts, typename IndexCalc>
struct ROCmStatementListExecutor<Data, StatementList<Stmts...>, IndexCalc> {

  using enclosed_stmts_t =
      camp::tuple<ROCmStatementExecutor<Data, Stmts, IndexCalc>...>;
                              ...
                              int block_carry)
  {
    ...
    ROCmStatementListExecutorHelper<0, sizeof...(Stmts)>::exec(
                                    ...
                                    int block_stride)
  {
    ...
    // Execute statements in order with helper class
    ROCmStatementListExecutorHelper<0, sizeof...(Stmts)>::initBlocks(
  ...
  LaunchDim calculateDimensions(Data const &data, LaunchDim const &max_physical)
  {
    ...
    // Compute this statements launch dimensions
    return ROCmStatementListExecutorHelper<0, sizeof...(Stmts)>::
