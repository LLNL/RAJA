...
namespace RAJA
{
  ...
  template<>
  class TensorRegister<avx_register, float, VectorLayout, camp::idx_seq<8>> :
    public internal::VectorRegisterBase<TensorRegister<avx_register, float, VectorLayout, camp::idx_seq<8>>>
  {
    public:
      using register_policy = avx_register;
      using self_type = TensorRegister<avx_register, float, VectorLayout, camp::idx_seq<8>>;
      using element_type = float;
      using register_type = __m256;

      using int_vector_type = TensorRegister<avx_register, int, VectorLayout, camp::idx_seq<8>>;
