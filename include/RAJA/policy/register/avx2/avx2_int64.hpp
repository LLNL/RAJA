...
namespace RAJA
{
  ...
  template<size_t N>
  class Register<vector_avx2_register, long, N> :
    public internal::RegisterBase<Register<vector_avx2_register, long, N>>
  {
    static_assert(N >= 1, "Vector must have at least 1 lane");
    static_assert(N <= 4, "AVX2 can only have 4 lanes of int64s");

    public:
      using self_type = Register<vector_avx2_register, long, N>;
