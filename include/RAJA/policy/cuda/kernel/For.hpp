...
struct CudaStatementExecutor<
  ...
  LaunchDims calculateDimensions(Data const &data)
  {
    diff_t len = segment_length<ArgumentId>(data);
    diff_t block_dim = DimBlockSize;
    diff_t blocks = RAJA_DIVIDE_CEILING_INT(len, block_dim);

    // request one thread and block per element in the segment
    LaunchDims dims;
    set_cuda_dim<GlobalDim>(dims.threads, block_dim);
    set_cuda_dim<GlobalDim>(dims.blocks, blocks);

    // since we are direct-mapping, we REQUIRE threads and blocks
    set_cuda_dim<GlobalDim>(dims.min_threads, block_dim);
    set_cuda_dim<GlobalDim>(dims.min_blocks, blocks);
