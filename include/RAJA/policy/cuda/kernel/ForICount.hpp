...
/*
 * Executor for global work sharing inside CudaKernel.
 * Provides a direct mapping of each global thread in xyz.
 * Assigns the loop index to offset ArgumentId
 * Assigns the loop index to param ParamId
 */
template <typename Data,
...
struct CudaStatementExecutor<
    Data,
    statement::ForICount<ArgumentId, ParamId, RAJA::cuda_global_xyz_direct<GlobalDim, DimBlockSize>, EnclosedStmts...>,
    Types>
    : public CudaStatementExecutor<
        Data,
        statement::For<ArgumentId, RAJA::cuda_global_xyz_direct<GlobalDim, DimBlockSize>, EnclosedStmts...>,
        Types> {
  ...
  inline RAJA_DEVICE void exec(Data &data, bool thread_active)
  {
    // grid stride loop
    diff_t len = segment_length<ArgumentId>(data);
    diff_t block_idx = get_cuda_dim<GlobalDim>(blockIdx);
    diff_t thread_idx = get_cuda_dim<GlobalDim>(threadIdx);
    diff_t block_dim = DimBlockSize;
    diff_t i = thread_idx + block_idx * block_dim;
...
/*
 * Executor for global work sharing inside CudaKernel.
 * Provides a grid-stride loop (stride of blockDim.xyz*gridDim.xyz) for
 * each global thread in xyz.
 * Assigns the loop index to offset ArgumentId
 * Assigns the loop index to param ParamId
 */
template <typename Data,
...
struct CudaStatementExecutor<
    Data,
    statement::ForICount<ArgumentId, ParamId, RAJA::cuda_global_xyz_loop<GlobalDim, DimBlockSize>, EnclosedStmts...>,
    Types>
    : public CudaStatementExecutor<
        Data,
        statement::For<ArgumentId, RAJA::cuda_global_xyz_loop<GlobalDim, DimBlockSize>, EnclosedStmts...>,
        Types> {
  ...
  inline RAJA_DEVICE void exec(Data &data, bool thread_active)
  {
    ...
    diff_t len = segment_length<ArgumentId>(data);
    diff_t block_idx = get_cuda_dim<GlobalDim>(blockIdx);
    diff_t grid_dim = get_cuda_dim<GlobalDim>(gridDim);
    diff_t thread_idx = get_cuda_dim<GlobalDim>(threadIdx);
    diff_t block_dim = get_cuda_dim<GlobalDim>(blockDim);
    diff_t i_init = thread_idx + block_idx * block_dim;
    diff_t i_stride = block_dim * grid_dim;
