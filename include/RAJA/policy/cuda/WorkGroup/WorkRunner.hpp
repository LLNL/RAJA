        ...
        Args...>
{
  ...
  per_run_storage run(WorkContainer const& storage, Args... args) const
  {
    ...
    RAJA::cuda::detail::LaunchInfo launch_info{
          cuda_dim_t{ static_cast<cuda_dim_member_t>((average_iterations + block_size - 1) / block_size),
                      static_cast<cuda_dim_member_t>(num_loops),
                      static_cast<cuda_dim_member_t>(1) },
          cuda_dim_t{ static_cast<cuda_dim_member_t>(block_size),
                      static_cast<cuda_dim_member_t>(1),
                      static_cast<cuda_dim_member_t>(1) },
          0,
          0
    ...
    if ( launch_info.gridDim.x  > zero && launch_info.gridDim.y  > zero && launch_info.gridDim.z  > zero &&
...

      {
        ...
        RAJA::cuda::launch((const void*)func, launch_info, func_args);
      }
