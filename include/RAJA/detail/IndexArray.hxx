...
struct index_storage {
    ...
    RAJA_INLINE
    constexpr
    auto get_data(StorageType& s) -> decltype(s.get()){
        return s.get();
    }
    template<typename StorageType>
    RAJA_HOST_DEVICE
    RAJA_INLINE
    constexpr
    auto get_data( const StorageType& s) -> decltype(s.get()){
        return s.get();
    }
...
struct select_element {
    ...
    static constexpr return_type get(AType_in& a, size_t offset) {
        return (offset == I) ?
            get_data<index_storage<I, value_type>>(a)
    ...
    static constexpr const_return_type get(const AType_in& a, size_t offset) {
        return (offset == I) ?
            get_data<const index_storage<I, value_type>>(a)
          : select_element<I - 1, AType_in>::get(a, offset);
    }
};
...
struct index_array_helper<Type, VarOps::index_sequence<orest...>> : index_storage<orest, Type>... {
    using type = Type;
...
template<typename Type, size_t ... orest>
constexpr size_t index_array_helper<Type, VarOps::index_sequence<orest...>>::size;

}

template<size_t Size, typename Type>
struct index_array : public detail::index_array_helper<Type, VarOps::make_index_sequence<Size>> {
    static_assert(Size > 0, "index_arrays must have at least one element");
    using base = detail::index_array_helper<Type, VarOps::make_index_sequence<Size>>;
...
RAJA_HOST_DEVICE
RAJA_INLINE
Type & get(detail::index_storage<Offset, Type>& s) {
    return s.data;
}
...
RAJA_HOST_DEVICE
RAJA_INLINE
const Type & get(const detail::index_storage<Offset, Type>& s) {
    return s.data;
}
...
template<typename Type,
    ...
    RAJA_HOST_DEVICE
    RAJA_INLINE
auto make_index_array_helper(VarOps::index_sequence<Seq...>, Args ... args)
    -> index_array<sizeof...(args), Type> {
...
template<typename Arg1,
    ...
    RAJA_HOST_DEVICE
    RAJA_INLINE
auto make_index_array(Arg1 arg1, Args ... args) -> index_array<sizeof...(args)+1, Arg1> {
    return detail::make_index_array_helper<Arg1>(VarOps::make_index_sequence<sizeof...(args)+1>(), arg1, args...);
};

template<size_t Size, typename Type>
std::ostream &operator<<(std::ostream &os, index_array<Size, Type> const &a) {
    // const detail::index_array_helper<Type, VarOps::make_index_sequence<Size>> & ah = a;
