...
RUN mkdir build && \
    cd build && \
    cmake -DCMAKE_CXX_COMPILER=g++ -DRAJA_ENABLE_WARNINGS=On -DRAJA_ENABLE_TBB=On -DRAJA_DEPRECATED_TESTS=On .. && \
    make -j 6 &&\
...
RUN mkdir build && \
    cd build && \
    cmake -DCMAKE_CXX_COMPILER=g++ -DRAJA_ENABLE_WARNINGS=On -DRAJA_ENABLE_WARNINGS_AS_ERRORS=On -DENABLE_COVERAGE=On -DRAJA_ENABLE_TBB=On .. && \
    make -j 6 &&\
    ctest -T test --output-on-failure
...
RUN mkdir build && \
    cd build && \
    cmake -DCMAKE_CXX_COMPILER=g++ -DRAJA_ENABLE_WARNINGS=On -DRAJA_ENABLE_TBB=On -DRAJA_ENABLE_RUNTIME_PLUGINS=On .. && \
    make -j 6 &&\
    ctest -T test --output-on-failure
...
RUN mkdir build && \
    cd build && \
    cmake -DCMAKE_CXX_COMPILER=g++ -DCMAKE_CXX_COMPILER=g++ -DRAJA_ENABLE_WARNINGS=On -DRAJA_ENABLE_TBB=On -DRAJA_ENABLE_BOUNDS_CHECK=ON .. && \
    make -j 6 &&\
    ctest -T test --output-on-failure
...
RUN mkdir build && \
    cd build && \
    cmake -DCMAKE_CXX_COMPILER=clang++ -DRAJA_ENABLE_TBB=On .. && \
    make -j 6 &&\
    ctest -T test --output-on-failure
