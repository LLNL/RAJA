...
// of RAJA's atomic implementations with Desul's atomic implementations.  In
// order to accomplish this without requiring two separate build system
// configurations this file directly includes "desul/atomics.hpp" and directly
// calls desul namespace atomics.  This is different from how a typical RAJA
// user would call a desul atomic.
...
// Desul atomics have a different signature than RAJA's built in ops. The
// following code provides some helper function templates so that they can be
// called using the same signature in timing code.
...
/// RAJA::atomicAdd and other RAJA namespace atomic calls are overloaded and
/// have an ambiguous type so they can't be passed as a template parameter. The
/// following macro disambiguates the call and provide a signature comaptible
/// with the DESUL wrapper. AtomicOperation must be a valid RAJA namespace
/// atomic operation, like atomicAdd, atomicMax, etc.
...
/// Instead of complicating the above macro to handle these two atomics, do the
/// declarations manually below.
template <typename Policy>
struct AtomicStore
{
...
/// Functor wrapping the desul implementation.  Wrapping the desul call ensure
/// an identical signature with RAJA's implementations.  Wrapping the call in an
/// functor allows simple type deduction for printing from within the benchmark.
template <
    typename T,
    typename ReturnType,
    typename DesulAtomicSignature<ReturnType, T*, const T, T>::type atomic_impl>
struct atomicWrapperDesulTernary
{
  /// Call operator overload template that allows invoking DESUL atomic with a
  /// (int*)(T*, T) signature
...
template <
    typename T,
    typename ReturnType,
    typename DesulAtomicSignature<ReturnType, T*, const T>::type atomic_impl>
...
template <typename T,
          typename ReturnType,
          typename DesulAtomicSignature<ReturnType, T*>::type atomic_impl>
...
template <typename T,
          typename ReturnType,
          typename DesulAtomicSignature<ReturnType, T*, T, T>::type atomic_impl>
class IsDesul<atomicWrapperDesulTernary<T, ReturnType, atomic_impl>>
    : public std::true_type
{};

template <typename T,
          typename ReturnType,
          typename DesulAtomicSignature<ReturnType, T*, T>::type atomic_impl>
class IsDesul<atomicWrapperDesulBinary<T, ReturnType, atomic_impl>>
    : public std::true_type
{};
...
template <typename T,
          typename ReturnType,
          typename DesulAtomicSignature<ReturnType, T*>::type atomic_impl>
class IsDesul<atomicWrapperDesulUnary<T, ReturnType, atomic_impl>>
    : public std::true_type
{};
...
template <class ExecPolicy,
          typename AtomicType,
          bool test_array,
          typename AtomicImplType>
void TimeAtomicOp(const AtomicImplType& atomic_impl,
                  uint64_t N,
                  uint64_t num_iterations = 4,
                  int array_size          = 100,
                  bool print_to_output    = true)
{
  RAJA::Timer timer;
  ...
  camp::resources::Resource resource {RAJA::resources::get_resource<
      typename ExecPolicy::policy>::type::get_default()};
  ...
  if (test_array)
  {
    ...
    {
      RAJA::forall<typename ExecPolicy::policy>(
          RAJA::TypedRangeSegment<uint64_t>(0, N),
...
/// Holder for atomic operations that work with arbitrary atomic type, e.g.
/// double, float and ints etc.
template <typename AtomicDataType, typename Policy>
struct universal_atomic_ops
{
  using type = camp::list<
      std::pair<atomicWrapperDesulBinary<AtomicDataType,
                                         AtomicDataType,
      ...
      std::pair<atomicWrapperDesulBinary<AtomicDataType,
                ...
                AtomicIncBinary<Policy>>,
      std::pair<atomicWrapperDesulBinary<AtomicDataType,
                                         AtomicDataType,
                                         desul::atomic_fetch_dec_mod>,
                AtomicDecBinary<Policy>>,
      std::pair<atomicWrapperDesulUnary<AtomicDataType,
                                        AtomicDataType,
                                        desul::atomic_fetch_inc>,
                AtomicIncUnary<Policy>>,
      std::pair<atomicWrapperDesulUnary<AtomicDataType,
                                        AtomicDataType,
                                        desul::atomic_fetch_dec>,
                AtomicDecUnary<Policy>>,
      std::pair<atomicWrapperDesulUnary<const AtomicDataType,
                                        AtomicDataType,
                                        desul::atomic_load>,
                AtomicLoad<Policy>>,
      std::pair<
          atomicWrapperDesulBinary<AtomicDataType, void, desul::atomic_store>,
          AtomicStore<Policy>>,
      std::pair<atomicWrapperDesulBinary<AtomicDataType,
                                         AtomicDataType,
                                         desul::atomic_exchange>,
                AtomicExchange<Policy>>,
      std::pair<atomicWrapperDesulTernary<AtomicDataType,
                                          AtomicDataType,
...
{
  using type =
      camp::list<std::pair<atomicWrapperDesulBinary<AtomicDataType,
                                                    AtomicDataType,
...
struct atomic_ops<
    AtomicDataType,
    Policy,
    typename std::enable_if<std::is_integral<AtomicDataType>::value>::type>
{
  using type = typename list_concat<
      typename universal_atomic_ops<AtomicDataType, Policy>::type,
      typename integral_atomic_ops<AtomicDataType, Policy>::type>::type;
};

/// Omit bitwise ops and, or, and xor for floating point types
template <typename AtomicDataType, typename Policy>
struct atomic_ops<AtomicDataType,
                  Policy,
                  typename std::enable_if<
                      std::is_floating_point<AtomicDataType>::value>::type>
{
...
void ExecuteBenchmark(uint64_t N)
{
  ...
  RAJA::for_each_type(
      ...
      [&](auto type_pair)
      {
        ...
        std::cout << INDENT << "Executing " << raja_functor.name
                  << " integer benchmarks" << std::endl;
        TimeAtomicOp<ExecPolicy, AtomicDataType, true>(desul_functor, N, 100,
                                                       10000);
        TimeAtomicOp<ExecPolicy, AtomicDataType, true>(raja_functor, N, 100,
                                                       10000);
        TimeAtomicOp<ExecPolicy, AtomicDataType, true>(desul_functor, N, 10,
                                                       1000);
        TimeAtomicOp<ExecPolicy, AtomicDataType, true>(raja_functor, N, 10,
                                                       1000);
...
int main(int argc, char* argv[])
{
  if (argc > 2)
  {
    RAJA_ABORT_OR_THROW("Usage: ./benchmark-atomic.exe <N> where N is the "
                        "optional size of the benchmark loop");
  }
  ...
  TimeAtomicOp<ExecPolicyGPU<BLOCK_SZ>, int, true>(
      atomicWrapperDesulBinary<int, int, desul::atomic_fetch_add> {}, N, 10,
      1000, false);
  TimeAtomicOp<ExecPolicyGPU<BLOCK_SZ>, int, true>(
      AtomicAdd<typename GPUAtomic::policy> {}, N, 10, 1000, false);
  // GPU benchmarks
  std::cout << "Executing GPU benchmarks" << std::endl;
  ExecuteBenchmark<double, typename GPUAtomic::policy, ExecPolicyGPU<BLOCK_SZ>>(
      N);
#endif

#if defined(RAJA_ENABLE_OPENMP)
  // Perform an untimed initialization of both desul and RAJA atomics.
  TimeAtomicOp<ExecPolicyOMP, int, true>(
      AtomicAdd<typename RAJA::policy::omp::omp_atomic> {}, N, 10, 1000, false);
  TimeAtomicOp<ExecPolicyOMP, int, true>(
      atomicWrapperDesulBinary<int, int, desul::atomic_fetch_add> {}, N, 10,
      1000, false);
  ...
  ExecuteBenchmark<double, typename RAJA::policy::omp::omp_atomic,
                   ExecPolicyOMP>(N);
#endif
