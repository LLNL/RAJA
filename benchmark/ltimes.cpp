...
void checkResult(PHIVIEW_T& phi,
                 LVIEW_T& L,
                 PSIVIEW_T& psi,
                 const int num_m,
                 const int num_d,
...
int main(int RAJA_UNUSED_ARG(argc), char** RAJA_UNUSED_ARG(argv[]))
{
...
#if VARIANT_C
  {
    ...
    std::cout << "  C-version of LTimes run time (sec.): " << t
              << ", GFLOPS/sec: " << gflop_rate << std::endl;
  }
#endif
...
#if VARIANT_C_VIEWS
  {
    ...
    LView L(L_data, RAJA::make_permuted_layout({{num_m, num_d}}, L_perm));
    ...
    double t          = timer.elapsed();
    double gflop_rate = total_flops / t / 1.0e9;
    std::cout << "  C-version of LTimes run time (with Views) (sec.): " << t
              << ", GFLOPS/sec: " << gflop_rate << std::endl;
...
#if VARIANT_RAJA_SEQ
  {
    ...
    using EXECPOL = RAJA::KernelPolicy<statement::For<
        2, seq_exec,                                                // g
    ...
    for (int iter = 0; iter < num_iter; ++iter)
      RAJA::kernel<EXECPOL>(segments, [=](IM m, ID d, IG g, IZ z)
                            { phi(m, g, z) += L(m, d) * psi(d, g, z); });

    timer.stop();
    double t          = timer.elapsed();
    double gflop_rate = total_flops / t / 1.0e9;
    std::cout << "  RAJA sequential version of LTimes run time (sec.): " << t
              << ", GFLOPS/sec: " << gflop_rate << std::endl;
...
#if VARIANT_RAJA_TEAMS_SEQ
  {
    ...
    for (int iter = 0; iter < num_iter; ++iter)
    {
      RAJA::launch<pol_launch>(
          ...
          {
            RAJA::loop<pol_g>(
                ctx, RAJA::TypedRangeSegment<IG>(0, num_g),
                [&](IG g)
                {
                  RAJA::loop<pol_z>(
                      ctx, RAJA::TypedRangeSegment<IZ>(0, num_z),
                      [&](IZ z)
                      {
                        RAJA::loop<pol_m>(
                            ctx, RAJA::TypedRangeSegment<IM>(0, num_m),
                            [&](IM m)
                            {
                              RAJA::loop<pol_d>(
                                  ctx, RAJA::TypedRangeSegment<ID>(0, num_d),
                                  [&](ID d)
                                  { phi(m, g, z) += L(m, d) * psi(d, g, z); });
                            });
                      });
...
#if VARIANT_RAJA_VECTOR
  {
    ...
    double t          = timer.elapsed();
    double gflop_rate = total_flops / t / 1.0e9;
    std::cout << "  RAJA vectorized version of LTimes run time (sec.): " << t
              << ", GFLOPS/sec: " << gflop_rate << std::endl;
...
#if VARIANT_RAJA_MATRIX
  {
    ...
    using matrix_t =
        RAJA::expt::SquareMatrixRegister<double, ColMajorLayout,
                                         RAJA::expt::scalar_register>;
    // using matrix_t = RAJA::expt::SquareMatrixRegister<double,
    // RowMajorLayout>;
    //  using matrix_t = RAJA::expt::RectMatrixRegister<double,
    //  RAJA::ColMajorLayout, 8,8>;


    std::cout << "matrix size: " << matrix_t::s_dim_elem(0) << "x"
              << matrix_t::s_dim_elem(1) << std::endl;
    ...
    for (int iter = 0; iter < num_iter; ++iter)
    {
      ...
      RAJA::forall<seq_exec>(RAJA::TypedRangeSegment<IG>(0, num_g),
                             [=](IG g)
                             {
                               ...
                               //      phi(rows_m, g, cols_z) = (L(rows_m,
                               //      cols_d) * psi(rows_d, g, cols_z)) *
                               //      (L(rows_m, cols_d) * psi(rows_d, g,
                               //      cols_z));
                             });
    }
    ...
    double gflop_rate = total_flops / t / 1.0e9;
    std::cout
        << "  RAJA column-major matrix version of LTimes run time (sec.): " << t
        << ", GFLOPS/sec: " << gflop_rate << std::endl;
...
//----------------------------------------------------------------------------//
#if VARIANT_RAJA_SEQ_SHMEM
  {
    ...
    using EXECPOL = RAJA::KernelPolicy<
        ...
        statement::InitLocalMem<
            RAJA::cpu_tile_mem, RAJA::ParamList<0, 1, 2>,

            // Tile outer m,d loops
            statement::Tile<
                0, tile_fixed<tile_m>, seq_exec,  // m
                statement::Tile<
                    1, tile_fixed<tile_d>, seq_exec,  // d
    ...
    using shmem_L_t =
        RAJA::TypedLocalArray<double, RAJA::PERM_JI,
                              RAJA::SizeList<tile_m, tile_d>, IM, ID>;
    shmem_L_t shmem_L;


    using shmem_psi_t =
        RAJA::TypedLocalArray<double, RAJA::PERM_IJK,
                              RAJA::SizeList<tile_d, tile_g, tile_z>, ID, IG,
                              IZ>;
    ...
    using shmem_phi_t =
        RAJA::TypedLocalArray<double, RAJA::PERM_IJK,
                              RAJA::SizeList<tile_m, tile_g, tile_z>, IM, IG,
                              IZ>;
    ...
    for (int iter = 0; iter < num_iter; ++iter)
      RAJA::kernel_param<EXECPOL>(
          ...
          RAJA::make_tuple(RAJA::TypedRangeSegment<IM>(0, num_m),
                           ...
                           RAJA::TypedRangeSegment<IG>(0, num_g),
                           RAJA::TypedRangeSegment<IZ>(0, num_z)),
          // For kernel_param, second arg is a tuple of data objects used in
          // lambdas. They are the last args in all lambdas (after indices).
          RAJA::make_tuple(shmem_L, shmem_psi, shmem_phi),


          // Lambda<0> : Load L into shmem
          [=](IM m, ID d, shmem_L_t& sh_L, IM tm, ID td)
          { sh_L(tm, td) = L(m, d); },

          // Lambda<1> : Load psi into shmem
          [=](ID d, IG g, IZ z, shmem_psi_t& sh_psi, ID td, IG tg, IZ tz)
          { sh_psi(td, tg, tz) = psi(d, g, z); },

          // Lambda<2> : Load phi into shmem
          [=](IM m, IG g, IZ z, shmem_phi_t& sh_phi, IM tm, IG tg, IZ tz)
          { sh_phi(tm, tg, tz) = phi(m, g, z); },

          // Lambda<3> : Compute phi in shmem
          [=](shmem_L_t& sh_L, shmem_psi_t& sh_psi, shmem_phi_t& sh_phi, IM tm,
              ID td, IG tg, IZ tz)
...
#if defined(RAJA_ENABLE_OPENMP) && (VARIANT_RAJA_OPENMP)
  {
    ...
    //
    using EXECPOL = RAJA::KernelPolicy<statement::Collapse<
        omp_parallel_collapse_exec, RAJA::ArgList<0, 2, 3>,  // m, g, z
        statement::For<1, seq_exec,                          // d
                       statement::Lambda<0>>>>;
#endif
...
#if VARIANT_CUDA_KERNEL
  {
    ...
    std::array<RAJA::idx_t, 2> L_perm {{0, 1}};
    LView L(dL_data, RAJA::make_permuted_layout({{num_m, num_d}}, L_perm));
    ...
    using EXECPOL =
        RAJA::KernelPolicy<statement::CudaKernelAsync<statement::For<
            0, cuda_block_x_loop,  // m
            statement::For<
                2, cuda_block_y_loop,                       // g
    ...
    for (int iter = 0; iter < num_iter; ++iter)
      RAJA::kernel<EXECPOL>(segments, [=] RAJA_DEVICE(IM m, ID d, IG g, IZ z)
                            { phi(m, g, z) += L(m, d) * psi(d, g, z); });
...
#if VARIANT_CUDA_TEAMS
  {
    ...
    using pol_launch =
        RAJA::LaunchPolicy<RAJA::seq_launch_t, RAJA::cuda_launch_t<true, 512>>;
    ...
    for (int iter = 0; iter < num_iter; ++iter)
    {
      RAJA::launch<pol_launch>(
          ...
          RAJA::LaunchParams(RAJA::Teams(160, 1, 1), RAJA::Threads(8, 64, 1)),
...
#define RAJA_GET_POLICY(POL) typename POL::host_policy_t
...
#if VARIANT_CUDA_TEAMS_MATRIX
  {
    ...
    using L_matrix_host_t =
        RAJA::expt::SquareMatrixRegister<double, matrix_layout>;
    using L_matrix_device_t =
        RAJA::expt::RectMatrixRegister<double, matrix_layout, 8, 4,
                                       RAJA::expt::cuda_warp_register>;
    using L_matrix_hd_t =
        RAJA::LaunchPolicy<L_matrix_host_t, L_matrix_device_t>;

    using phi_matrix_host_t =
        RAJA::expt::SquareMatrixRegister<double, matrix_layout>;
    using phi_matrix_device_t =
        RAJA::expt::RectMatrixRegister<double, matrix_layout, 8, 8,
                                       RAJA::expt::cuda_warp_register>;
    using phi_matrix_hd_t =
        RAJA::LaunchPolicy<L_matrix_host_t, phi_matrix_device_t>;

    using psi_matrix_host_t =
        RAJA::expt::SquareMatrixRegister<double, matrix_layout>;
    using psi_matrix_device_t =
        RAJA::expt::RectMatrixRegister<double, matrix_layout, 4, 8,
                                       RAJA::expt::cuda_warp_register>;
    using psi_matrix_hd_t =
        RAJA::LaunchPolicy<L_matrix_host_t, psi_matrix_device_t>;


    using pol_launch =
        RAJA::LaunchPolicy<RAJA::seq_launch_t, RAJA::cuda_launch_t<true, 1024>>;
    ...
    for (int iter = 0; iter < num_iter; ++iter)
    {
      RAJA::launch<pol_launch>(
          ...
          [=] RAJA_HOST_DEVICE(RAJA::LaunchContext ctx)
          {
            ...
            RAJA::loop<pol_g>(ctx, RAJA::TypedRangeSegment<IG>(0, num_g),
                              [&](IG g)
                              {
                                RAJA::tile<pol_z>(
                                    ctx, 32,
                                    RAJA::TypedRangeSegment<int>(0, num_z),
                                    [&](RAJA::TypedRangeSegment<int> tzi)
                                    {
                                      RAJA::TypedRangeSegment<IZ> tz(
                                          *tzi.begin(), *tzi.end());

                                      phi(phi_RowM::all(), g, phi_ColZ(tz)) +=
                                          L(L_RowM::all(), L_ColD::all()) *
                                          psi(psi_RowD::all(), g, psi_ColZ(tz));
                                    });
                              });
          });
    }
...
#if VARIANT_CUDA_KERNEL_SHMEM
  {
    ...
    using shmem_L_t =
        RAJA::TypedLocalArray<double, RAJA::PERM_IJ,
                              RAJA::SizeList<tile_m, tile_d>, IM, ID>;
    shmem_L_t shmem_L;
    ...
    using EXECPOL =
        RAJA::KernelPolicy<statement::CudaKernelAsync<statement::InitLocalMem<
            cuda_shared_mem, ParamList<0, 1>,
            // Tile outer m,d loops
            statement::Tile<
                0, tile_fixed<tile_m>, seq_exec,  // m
                statement::Tile<
                    ...
                    statement::For<
                        ...
                        statement::Tile<
                            ...
                            statement::For<
                                3, cuda_thread_x_direct,  // z
                                statement::For<
                                    0, cuda_thread_y_direct,  // m
    ...
    for (int iter = 0; iter < num_iter; ++iter)
      RAJA::kernel_param<EXECPOL>(
          ...
          // computing a phi value, for shared memory before writing to phi
          // array.
          RAJA::make_tuple(shmem_L, shmem_psi, 0.0),

          // Lambda<0> : Load L into shmem
          [=] RAJA_DEVICE(IM m, ID d, shmem_L_t & sh_L, IM tm, ID td)
          { sh_L(tm, td) = L(m, d); },

          // Lambda<1> : Load slice of psi into shmem
          [=] RAJA_DEVICE(ID d, IG g, IZ z, shmem_psi_t & sh_psi, ID td, IG tg,
                          IZ tz) { sh_psi(td, tg, tz) = psi(d, g, z); },

          // Lambda<2> : Load thread-local phi value
          [=] RAJA_DEVICE(IM m, IG g, IZ z, double& phi_local)
          { phi_local = phi(m, g, z); },

          // Lambda<3> Compute thread-local phi value
          [=] RAJA_DEVICE(IM m, ID d, IG g, IZ z, shmem_L_t & sh_L,
                          shmem_psi_t & sh_psi, double& phi_local, IM tm, ID td,
                          IG tg, IZ tz)
...
#if RAJA_HIP_KERNEL
  {
    ...
    using EXECPOL = RAJA::KernelPolicy<statement::HipKernelAsync<statement::For<
        0, hip_block_x_loop,                                       // m
...
#if RAJA_HIP_KERNEL_SHMEM
  {
    ...
    using EXECPOL =
        RAJA::KernelPolicy<statement::HipKernelAsync<statement::InitLocalMem<
            hip_shared_mem, ParamList<0, 1>,
            // Tile outer m,d loops
            statement::Tile<
                0, tile_fixed<tile_m>, seq_exec,  // m
                statement::Tile<
                    ...
                    statement::For<
                        ...
                        statement::Tile<
                            ...
                            // Load phi into thread local storage
                            statement::For<
                                3, hip_thread_x_direct,  // z
                                statement::For<
                                    0, hip_thread_y_direct,  // m
                                    statement::Lambda<2, Segs<0, 2, 3>,
                                                      Params<2>>>>,
                            ...
                            statement::For<
                                3, hip_thread_x_direct,               // z
                                statement::For<1, hip_thread_y_loop,  // d
                                                                      // (reusing
                                                                      // y)
                                               statement::Lambda<
                                                   1, Segs<1, 2, 3>, Params<1>,
    ...
    for (int iter = 0; iter < num_iter; ++iter)
      RAJA::kernel_param<EXECPOL>(
          ...
          [=] RAJA_DEVICE(IM RAJA_UNUSED_ARG(m), ID RAJA_UNUSED_ARG(d),
                          ...
                          shmem_L_t & sh_L, shmem_psi_t & sh_psi,
                          double& phi_local, IM tm, ID td, IG tg, IZ tz)
          { phi_local += sh_L(tm, td) * sh_psi(td, tg, tz); },
