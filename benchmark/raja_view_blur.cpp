...
using kernel_pol = RAJA::KernelPolicy<RAJA::statement::HipKernelFixed<
    ...
    RAJA::statement::For<
        1,
        RAJA::hip_global_size_y_direct<16>,
        RAJA::statement::For<0,
                             RAJA::hip_global_size_x_direct<16>,
...
using kernel_pol = RAJA::KernelPolicy<RAJA::statement::CudaKernelFixed<
    256,
    RAJA::statement::For<
        1,
        RAJA::cuda_global_size_y_direct<16>,
        RAJA::statement::For<0,
                             RAJA::cuda_global_size_x_direct<16>,
                             RAJA::statement::Lambda<0>>>>>;
#endif
...
using kernel_pol =
    RAJA::KernelPolicy<RAJA::statement::SyclKernel<RAJA::statement::For<
        ...
        RAJA::statement::
            For<0, RAJA::sycl_global_item_2, RAJA::statement::Lambda<0>>>>>;
#endif
...
using kernel_pol = RAJA::KernelPolicy<RAJA::statement::For<
    1,
...
int main(int RAJA_UNUSED_ARG(argc), char** RAJA_UNUSED_ARG(argv[]))
{
  ...
  RAJA::forall<device_pol>(RAJA::RangeSegment(0, 1),
  ...
  RAJA::View<int, RAJA::Layout<DIM, int, 1>> array_view_copy(d_array_copy, N,
                                                             N);
  ...
  RAJA::kernel<kernel_pol>(RAJA::make_tuple(range_i, range_j),
                           [=] RAJA_HOST_DEVICE(int i, int j)
                           {
                             ...
                             for (int m = 0; m < K; ++m)
                             {
                               for (int n = 0; n < K; ++n)
                               {
                                 ...
                                 // adding the "blur" to the "image" wherever
                                 // the blur is located on the image
                                 if (x < N && y < N)
                                 {
  ...
  std::cout << "Elapsed time with NO RAJA view : " << timer.elapsed()
            << std::endl;
