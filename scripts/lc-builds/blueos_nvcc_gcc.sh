...
if [[ $# -lt 3 ]]; then
  echo
  echo "You must pass at least 3 arguments to the script (in this order): "
  ...
  echo "   4) (optional) benchmarks On"
  echo
  echo "For example: "
  echo "    blueos_nvcc_gcc.sh 10.2.89 70 8.3.1 On"
  exit
fi
...
if [[ $# -gt 3 ]]; then 
  BENCHMARKS=$4
  shift 4
  BUILD_SUFFIX=lc_blueos-nvcc${COMP_NVCC_VER}-${COMP_ARCH}-gcc${COMP_GCC_VER}-benchmark_${BENCHMARKS}
else
  shift 3
  BENCHMARKS=Off
