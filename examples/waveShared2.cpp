...
void innerLoop(const serialLoop &, int Ny, int Nx, Func &&innerLoop)
{
  ...
  RAJA::forallN<RAJA::NestedPolicy<RAJA::ExecList<policyY, policyX>>>(
      RAJA::RangeSegment(0, Ny),
      RAJA::RangeSegment(0, Nx),
      std::forward<Func>(innerLoop));
}
...
__device__ void RAJA_Sync(const cudaBlockSync &) { __syncthreads(); }
...
int main(int RAJA_UNUSED_ARG(argc), char **RAJA_UNUSED_ARG(argv[]))
{
  ...
  printf(
      "Example 4. Time-Domain Finite Difference Acoustic Wave Equation Solver "
      "\n");
  ...
  */
  ...
  using pol = RAJA::nested::Policy<RAJA::nested::CudaKernel<
      1024,
      RAJA::nested::Collapse<RAJA::cuda_threadblock_exec,
                             RAJA::nested::ArgList<0, 1>,
                             RAJA::nested::Lambda<0>>>>;
#else
  using pol = RAJA::nested::Policy<RAJA::nested::For<0, RAJA::loop_exec>,
...
void computeErr(double *P, double tf, grid_s grid)
{
  ...
  RAJA::forallN<myPolicy>(fdBounds,
                          fdBounds,
...
void waveShared(T *P1, T *P2, RAJA::RangeSegment outerBounds, double ct, int nx)
{
...
#else
  RAJA::SharedMemoryView<RAJA::SharedMemory<RAJA::seq_shmem, double>,
                         RAJA::Layout<2>,
                         RAJA::ident_shmem,
                         RAJA::ident_shmem>
      Lu((Bx + 2 * sr), (By + 2 * sr));
  ...
  RAJA::nested::forall(fdNestedPolicy{},
                       camp::make_tuple(outerBounds, outerBounds),
                       [=] __device__(RAJA::Index_type outerIdy,
                                      RAJA::Index_type outerIdx) {
#else
  RAJA::nested::forall(fdNestedPolicy{},
                       camp::make_tuple(outerBounds, outerBounds),
                       [=](RAJA::Index_type outerIdy,
                           RAJA::Index_type outerIdx) {
#endif
                         /*
                           Coefficients for a fourth order stencil
                         */
                         double coeff[5] = {-1.0 / 12.0,
                                            4.0 / 3.0,
                                            -5.0 / 2.0,
                                            4.0 / 3.0,
                                            -1.0 / 12.0};

                         innerLoop<innerPolicyY, innerPolicyX>(
                             innerPolicy{}, By, Bx, [&](int ly, int lx) {
                               // Compute Index
