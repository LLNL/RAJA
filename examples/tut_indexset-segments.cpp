...
int main(int RAJA_UNUSED_ARG(argc), char** RAJA_UNUSED_ARG(argv[]))
{
  ...
  RAJA::forall<RAJA::seq_exec>(idx_list, [=](IdxType i) { a[i] += b[i] * c; });
  ...
  RAJA::forall<RAJA::seq_exec>(idx_reverse_list,
                               [=](IdxType i) { a[i] += b[i] * c; });
  ...
  // loop in reverse.
  //
  std::cout << "\n Running RAJA daxpy with indices reversed via negatively "
               "strided range segment...\n";
  ...
  RAJA::forall<SEQ_ISET_EXECPOL>(is1, [=](IdxType i) { a[i] += b[i] * c; });
  ...
  RAJA::forall<SEQ_ISET_EXECPOL>(is2, [=](IdxType i) { a[i] += b[i] * c; });
  ...
  std::cout << "\n Running RAJA index set (2 RangeSegments, 1 ListSegment) "
               "daxpy...\n";
  ...
  RAJA::forall<SEQ_ISET_EXECPOL>(is3, [=](IdxType i) { a[i] += b[i] * c; });
  ...
  std::cout << "\n Running RAJA index set (2 RangeSegments, 1 ListSegment) "
               "daxpy\n"
            << " (sequential iteration over segments, OpenMP parallel segment "
               "execution)...\n";

  std::memcpy(a, a0, N * sizeof(double));

  using OMP_ISET_EXECPOL1 =
      RAJA::ExecPolicy<RAJA::seq_segit, RAJA::omp_parallel_for_exec>;

  RAJA::forall<OMP_ISET_EXECPOL1>(is3, [=](IdxType i) { a[i] += b[i] * c; });
  ...
  std::cout << "\n Running RAJA index set (2 RangeSegments, 1 ListSegment) "
               "daxpy\n"
            << " (OpenMP parallel iteration over segments, sequential segment "
               "execution)...\n";

  std::memcpy(a, a0, N * sizeof(double));

  using OMP_ISET_EXECPOL2 =
      RAJA::ExecPolicy<RAJA::omp_parallel_for_segit, RAJA::seq_exec>;

  RAJA::forall<OMP_ISET_EXECPOL2>(is3, [=](IdxType i) { a[i] += b[i] * c; });
...
// printResult(a, N);
  ...
  std::cout << "\n Running RAJA index set (2 RangeSegments, 1 ListSegment) "
               "daxpy\n"
            << " (sequential iteration over segments, CUDA parallel segment "
               "execution)...\n";

  using OMP_ISET_EXECPOL3 =
