...
int main(int RAJA_UNUSED_ARG(argc), char **RAJA_UNUSED_ARG(argv[]))
{
  ...
  for (int exec_place = 0; exec_place < (int)RAJA::expt::NUM_PLACES; ++exec_place) {
    ...
    int N_tri = 5;
    int *Ddat;
    if(select_cpu_or_gpu == RAJA::expt::HOST) Ddat = host_res.allocate<int>(N_tri * N_tri);
    if(select_cpu_or_gpu == RAJA::expt::DEVICE) Ddat = device_res.allocate<int>(N_tri * N_tri);
    ...
    if(select_cpu_or_gpu == RAJA::expt::HOST) {host_res.deallocate(Ddat);}
    if(select_cpu_or_gpu == RAJA::expt::DEVICE) {device_res.deallocate(Ddat);}
