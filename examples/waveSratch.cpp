...
template <typename T, typename fdNestedPolicy>
void waveSharedCPU(T *P1, T *P2, RAJA::RangeSegment fdBounds, double ct, int nx);

//To be paired with wave Abstract
template <typename T, typename fdNestedPolicy>
void waveAbstract(T *P1, T *P2, RAJA::RangeSegment fdBounds, double ct, int nx);
...
int main(int RAJA_UNUSED_ARG(argc), char **RAJA_UNUSED_ARG(argv[]))
{
  ...
  */
  ...
  for (int k = 0; k < nt; ++k) {
    ...
    waveShared<double, sharedPolicy>(P1, P2, fdBounds, ct, grid.nx);
    time += dt;
  ...
  //No Shared Memory
  //---------
  std::cout<<"\n \n With no shared memory!"<<std::endl;
  // CUDA
  using sharedPolicy
     = RAJA::NestedPolicy<RAJA::ExecList
     <RAJA::cuda_threadblock_y_exec<By>,
  ...
  setIC(P1, P2, (time - dt), time, grid);
  for (int k = 0; k < nt; ++k) {
    wave<double, sharedPolicy>(P1, P2, fdBounds, ct, grid.nx);
    time += dt;
  ...
  //Outer loops will must be sequential
  using cpuPolicy =
  ...
  setIC(P1, P2, (time - dt), time, grid);
  for (int k = 0; k < nt; ++k) {
    waveSharedCPU<double, cpuPolicy>(P1, P2, outerBounds, ct, grid.nx);
    time += dt;
  ...
  setIC(P1, P2, (time - dt), time, grid);
  for (int k = 0; k < nt; ++k) {
    waveAbstract<double, abstractPolicy>(P1, P2, outerRange, ct, grid.nx);
...
//with shared memory
template <typename T, typename fdNestedPolicy>
void waveAbstract(T *P1, T *P2, RAJA::RangeSegment outerBounds, double ct, int nx)
{
...
void waveSharedCPU(T *P1, T *P2, RAJA::RangeSegment fdBounds, double ct, int nx)
{
  ...
  //RAJA Outer Loops
 RAJA::forallN<fdNestedPolicy>(
      fdBounds, fdBounds, [=] (RAJA::Index_type blocky, RAJA::Index_type blockx) {
        ...
        */
        ...
        RAJA_Par_Inner(ly, By) {
          RAJA_Inner(lx, Bx) {
            ...
            //Compute Index
            const int tx = lx + Bx * blockx; 
            const int ty = ly + By * blocky;                        
            const int id = ty*nx+ tx;             
...
void waveShared(T *P1, T *P2, RAJA::RangeSegment fdBounds, double ct, int nx)
{
 ...
 RAJA::forallN<fdNestedPolicy>(
      fdBounds, fdBounds, [=] __device__ (RAJA::Index_type ty, RAJA::Index_type tx) {
