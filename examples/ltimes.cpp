...
int main(int RAJA_UNUSED_ARG(argc), char** RAJA_UNUSED_ARG(argv[]))
{
...

  {
    ...
    timer.stop();
    std::cout << "  C-version of LTimes run time (sec.): " << timer.elapsed()
              << std::endl;
  }
...

  {
    ...
    using EXECPOL = RAJA::KernelPolicy<
        ...
        statement::Tile<
            0,
            statement::tile_fixed<tile_m>,
            loop_exec,  // m
            statement::Tile<
                1,
                statement::tile_fixed<tile_d>,
                loop_exec,  // d
    ...
    //
    using shmem_L_T = RAJA::ShmemTile<RAJA::cpu_shmem,
                                      double,
                                      RAJA::ArgList<0, 1>,
    ...
    using shmem_psi_T = RAJA::ShmemTile<RAJA::cpu_shmem,
                                        double,
                                        RAJA::ArgList<1, 2, 3>,
    ...
    using shmem_phi_T = RAJA::ShmemTile<RAJA::cpu_shmem,
                                        double,
                                        RAJA::ArgList<0, 2, 3>,
    ...
    RAJA::kernel_param<EXECPOL>(
        ...
        RAJA::make_tuple(sh_L, sh_psi, sh_phi),

        // Lambda<0> : Single lambda version
        [=](IM m, ID d, IG g, IZ z, shmem_L_T&, shmem_psi_T&, shmem_phi_T&) {
          phi(m, g, z) += L(m, d) * psi(d, g, z);
        },
        ...
        [=](IM /*m*/,
            ...
            shmem_L_T&,
            shmem_psi_T& sh_psi,
            shmem_phi_T&) { sh_psi(d, g, z) = psi(d, g, z); },
        ...
        [=](IM m,
            ...
            shmem_L_T&,
            shmem_psi_T&,
            shmem_phi_T& sh_phi) { sh_phi(m, g, z) = phi(m, g, z); },
        ...
        [=](IM m,
            ...
            shmem_L_T&,
            shmem_psi_T&,
            shmem_phi_T& sh_phi) { phi(m, g, z) = sh_phi(m, g, z); }
...
#if defined(RAJA_ENABLE_CUDA)
  {
    ...
    cudaErrchk(cudaMemcpy(
        dL_data, L_data, L_size * sizeof(double), cudaMemcpyHostToDevice));
    cudaErrchk(cudaMalloc((void**)&dpsi_data, psi_size * sizeof(double)));
    cudaErrchk(cudaMemcpy(dpsi_data,
                          psi_data,
                          psi_size * sizeof(double),
                          cudaMemcpyHostToDevice));
    cudaErrchk(cudaMalloc((void**)&dphi_data, phi_size * sizeof(double)));
    cudaErrchk(cudaMemcpy(dphi_data,
                          phi_data,
                          phi_size * sizeof(double),
                          cudaMemcpyHostToDevice));
    ...
    cudaErrchk(cudaMemcpy(phi_data,
                          dphi_data,
                          phi_size * sizeof(double),
...
#if defined(RAJA_ENABLE_CUDA)
  {
    ...
    LView L(dL_data, RAJA::make_permuted_layout({num_m, num_d}, dL_perm));
    ...
    using EXECPOL = RAJA::KernelPolicy<statement::CudaKernelAsync<
                            ...
                            statement::SetShmemWindow<
                                ...
                                statement::For<
                                    ...
                                    statement::For<
                                        ...
                                        // Compute thread-local Phi value and
                                        // store
                                        statement::Thread<
                                            statement::Lambda<3>,
                                            statement::For<
    ...
    // Define shared memory tiles used in kernel
    //
    using shmem_L_T = RAJA::ShmemTile<RAJA::cuda_shmem,
                                      double,
                                      RAJA::ArgList<1, 0>,
    ...
    using shmem_psi_T = RAJA::ShmemTile<RAJA::cuda_shmem,
                                        double,
                                        RAJA::ArgList<1, 3>,
    ...
    RAJA::kernel_param<EXECPOL>(
        ...
        // For kernel_param, second arg is a tuple of data objects used in
        // lambdas. They are the last args in all lambdas (after indices). Here,
        // the last entry '0.0' yields a thread-private temporary for computing
        // a phi value, for shared memory before writing to phi array.
        RAJA::make_tuple(shmem_L, shmem_psi, 0.0),

        // Lambda<0> : Single lambda version
        [=] RAJA_DEVICE(
            IM m, ID d, IG g, IZ z, shmem_L_T&, shmem_psi_T&, double&) {
          phi(m, g, z) += L(m, d) * psi(d, g, z);
        },
        ...
        [=] RAJA_DEVICE(IM m,
                        ...
                        shmem_L_T&,
                        shmem_psi_T&,
                        double& phi_local) { phi_local = phi(m, g, z); },
        ...
        [=] RAJA_DEVICE(IM m,
                        ...
                        shmem_L_T&,
                        shmem_psi_T&,
                        double& phi_local) { phi(m, g, z) = phi_local; }
