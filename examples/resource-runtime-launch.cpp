...
using device_loop = RAJA::sycl_global_item_0;
#endif
...
int main(int argc, char *argv[])
{
  ...
  if(exec_space.compare("host") == 0)
    { select_cpu_or_gpu = RAJA::HOST; printf("Running RAJA-Teams reductions example on the host \n"); }
  if(exec_space.compare("device") == 0)
    { select_cpu_or_gpu = RAJA::DEVICE; printf("Running RAJA-Teams reductions example on the device \n"); }
...
//----------------------------------------------------------------------------//
  ...
  RAJA::launch<launch_policy>
    (res, shared_mem, RAJA::Grid(RAJA::Teams(GRID_SZ),
