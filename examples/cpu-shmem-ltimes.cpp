...
void runLTimesRajaKernelVec(bool debug,
                          Index_type num_moments,
                          Index_type num_directions,
                          Index_type num_groups,
                          Index_type num_zones)
{
  ...
  using Pol = RAJA::KernelPolicy<
    For<0, loop_exec,
      For<1, loop_exec,
        For<2, loop_exec,
          For<3, vec_exec<VecType>, Lambda<0>>
        >
      >
    >>;


  auto vphi = RAJA::make_vector_view(phi);
  auto vell = RAJA::make_vector_view(ell);
  auto vpsi = RAJA::make_vector_view(psi);
  ...
  timer.stop();
  printf("LTimes took %lf seconds using RAJA::kernel and vec_exec\n",
      timer.elapsed());
...
int main(){
  ...
  runLTimesRajaKernelVec(!debug, m, d, g, z);
