...
to an array offset indexx by subtracting the lower offset from it; i.e., in 
...
enabling users to  specify integral index types. Basic usage requires 
specifying types for the linear index, and the multi-dimensional indicies. 
The following example creates typed layouts wherein the linear index is of 
type TIL and the multidimensional indices are TIX, TIY,::
...
well the value of the index and bounds. Since the bounds checking is a runtime
