...
optimizations for a loop, we would use the:: 

  RAJA::simd_exec

execution policy. Alternatively, we could use the::

  RAJA::loop_exec

execution policy, which allows the compiler to generate optimizations, such
as SIMD if compiler heuristics suggest that it is safe to do so and that 
there would be performance benefits.
