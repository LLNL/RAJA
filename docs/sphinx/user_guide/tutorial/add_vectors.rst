...
compiler to generated SIMD vectorization optimizations for a loop, we would 
use the RAJA SIMD execution policy:: 
...
``CUDA_BLOCK_SIZE``, which you can change in the example code. This value
specifies that each CUDA thread block that is launched to execute the kernel 
will have the given number threads in the block. The thread block size
