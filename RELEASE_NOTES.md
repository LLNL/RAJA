...
Notable changes include:
  ...
  * New features / API changes:
     ...
     * GPU global (thread and block) indexing has been refactored to abstract 
       ...
       a block size or a grid size at compile time or get those values at 
       run time. You can also ignore blocks and index only with threads and            vice versa. Kernel and launch policies are now shared. Such policies are
       ...
       complete the even if some have no work so you can synchronize a block.
       Aliases have been added for all of the pre-existing policies and some 
       are deprecated in favor more consistently named policies. One 
