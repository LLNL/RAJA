...
CUDA_TYPED_TEST_P(Nested, Basic)
{
  ...
  auto ranges =
      RAJA::make_tuple(RAJA::TypedRangeSegment<Idx0>(0, x_len),
...
using OMPTypes = ::testing::Types<
    list<Policy<For<1, RAJA::omp_parallel_for_exec>, For<0, s>>,
         list<TypedIndex, Index_type>,
         RAJA::omp_reduce>,
...
using TBBTypes =
    ::testing::Types<list<Policy<For<1, RAJA::tbb_for_exec>, For<0, s>>,
                          list<TypedIndex, Index_type>,
                          RAJA::tbb_reduce>>;
...
#if defined(RAJA_ENABLE_CUDA)
using CUDATypes =
    ::testing::Types<list<Policy<For<1, s>, For<0, RAJA::cuda_exec<128>>>,
                          list<TypedIndex, Index_type>,
...
TEST(Nested, TileDynamic)
{
  ...
  auto pol = RAJA::make_tuple(Tile<1, tile<2>, RAJA::seq_exec>{tile_size},
                              For<0, RAJA::seq_exec>{},
                              For<1, RAJA::seq_exec>{});
  auto ranges = RAJA::make_tuple(RAJA::RangeSegment(0, length),
                                 RAJA::RangeStrideSegment(0, length, 1));
  RAJA::nested::forall(pol, ranges, [=, &count](Index_type i, Index_type j) {
    //        std::cerr << "i: " << get_val(i) << " j: " <<
    //        j << " count: " << count
    //                  << std::endl;
...
CUDA_TEST(Nested, CudaCollapse)
{
  ...
  using Pol = RAJA::nested::Policy<RAJA::nested::CudaCollapse<
...
CUDA_TEST(Nested, CudaReduceC)
{
  ...
  using Pol = RAJA::nested::Policy<RAJA::nested::For<2, RAJA::loop_exec>,
...
TEST(Nested, Collapse2)
{
  ...
  using Pol = RAJA::nested::Policy<
      RAJA::nested::OmpParallelCollapse<RAJA::nested::For<0>,
                                        RAJA::nested::For<1>>>;
...
TEST(Nested, Collapse6)
{
  ...
  using Pol = RAJA::nested::Policy<
      ...
      RAJA::nested::OmpParallelCollapse<RAJA::nested::For<1>,
                                        RAJA::nested::For<2>>>;
