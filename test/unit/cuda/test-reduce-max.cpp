...
CUDA_TEST_F(ReduceMaxCUDA, generic)
{
  ...
  for (int tcount = 0; tcount < test_repeat; ++tcount) {
    ...
    ReduceMax<cuda_reduce_sync, double> dmax0; dmax0.reset(DEFAULT_VAL);
    ReduceMax<cuda_reduce_sync, double> dmax1(DEFAULT_VAL);
    ReduceMax<cuda_reduce_sync, double> dmax2(BIG_VAL);
...
CUDA_TEST_F(ReduceMaxCUDA, indexset_align)
{
  ...
  for (int tcount = 0; tcount < test_repeat; ++tcount) {
    ...
    ReduceMax<cuda_reduce_sync, double> dmax0(DEFAULT_VAL);
    ReduceMax<cuda_reduce_sync, double> dmax1(DEFAULT_VAL);
