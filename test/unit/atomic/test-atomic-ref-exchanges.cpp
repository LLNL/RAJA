...
GPU_TYPED_TEST_P( AtomicRefCUDAExchangeUnitTest, CUDAExchanges )
{
  ...
  // test exchange method
  forone_pol<forone_cuda>( [=] __device__ () {swapper[0] = test1.exchange( swapper[0] );} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( test1, (T)91 );
  ASSERT_EQ( swapper[0], (T)0 );

  // test CAS method
  forone_pol<forone_cuda>( [=] __device__ () {swapper[0] = test1.CAS( (T)91, swapper[0] );} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( test1, (T)0 );
  ASSERT_EQ( swapper[0], (T)91 );

  // test strong exchange method
  forone_pol<forone_cuda>( [=] __device__ () {result[0] = test1.compare_exchange_strong( testval[0], testval[0] );} );
  cudaErrchk(cudaDeviceSynchronize());
  ...
  // test weak exchange method (same as strong exchange)
  forone_pol<forone_cuda>( [=] __device__ () {result[0] = test1.compare_exchange_weak( testval[0], swapper[0] );} );
  cudaErrchk(cudaDeviceSynchronize());
