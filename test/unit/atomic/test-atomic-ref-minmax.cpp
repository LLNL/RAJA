...
GPU_TYPED_TEST_P( AtomicRefCUDAMinMaxUnitTest, CUDAMinMaxs )
{
  ...
  // test min
  forone_pol<forone_cuda>( [=] __device__ () {result[0] = test1.fetch_min( (T)87 );} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( result[0], (T)91 );
  ASSERT_EQ( test1, (T)87 );

  forone_pol<forone_cuda>( [=] __device__ () {result[0] = test1.min( (T)83 );} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( result[0], (T)83 );
  ASSERT_EQ( test1, (T)83 );

  // test max
  forone_pol<forone_cuda>( [=] __device__ () {result[0] = test1.fetch_max( (T)87 );} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( result[0], (T)83 );
  ASSERT_EQ( test1, (T)87 );

  forone_pol<forone_cuda>( [=] __device__ () {result[0] = test1.max( (T)91 );} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( result[0], (T)91 );
  ASSERT_EQ( test1, (T)91 );
