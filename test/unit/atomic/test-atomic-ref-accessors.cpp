...
GPU_TYPED_TEST_P( AtomicRefCUDAAccessorUnitTest, CUDAAccessors )
{
  ...
  // test store method with op()
  forone_pol<forone_cuda>( [=] __device__ () {test1.store( (T)19 );} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( test1, (T)19 );

  // test assignment operator
  forone_pol<forone_cuda>( [=] __device__ () {test1 = (T)23;} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( test1, (T)23 );

  // test load method
  forone_pol<forone_cuda>( [=] __device__ () {test1 = (T)29; result[0] = test1.load();} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( result[0], (T)29 );
  ASSERT_EQ( test1, (T)29 );

  // test T()
  forone_pol<forone_cuda>( [=] __device__ () {test1 = (T)47; result[0] = test1;} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( result[0], (T)47 );
  ASSERT_EQ( test1, (T)47 );

  forone_pol<forone_cuda>( [=] __device__ () {result[0] = (test1 = (T)31);} );
  cudaErrchk(cudaDeviceSynchronize());
