...
GPU_TYPED_TEST_P( AtomicRefCUDAAddSubUnitTest, CUDAAddSubs )
{
  ...
  // test inc ops
  forone_pol<forone_cuda>( [=] __device__ () {result1[0] = ++test1;} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( result1[0], (T)1 );
  forone_pol<forone_cuda>( [=] __device__ () {result2[0] = test1++;} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( test1, (T)2 );
  ...
  // test add/sub ops
  forone_pol<forone_cuda>( [=] __device__ () {result1[0] = (test1 += (T)23);} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( test1, (T)23 );
  ASSERT_EQ( result1[0], (T)23 );
  forone_pol<forone_cuda>( [=] __device__ () {result2[0] = (test1 -= (T)22);} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( test1, (T)1 );
  ASSERT_EQ( result2[0], (T)1 );

  // test add/sub methods
  forone_pol<forone_cuda>( [=] __device__ () {result1[0] = test1.fetch_add( (T)23 );} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( test1, (T)24 );
  ASSERT_EQ( result1[0], (T)1 );
  forone_pol<forone_cuda>( [=] __device__ () {result2[0] = test1.fetch_sub( (T)23 );} );
  cudaErrchk(cudaDeviceSynchronize());
  ASSERT_EQ( test1, (T)1 );
