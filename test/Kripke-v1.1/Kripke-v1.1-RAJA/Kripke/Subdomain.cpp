...
*/
void Subdomain::setup(int sdom_id,
  ...
  for (int dim = 0; dim < 3; ++dim) {
    ...
    std::pair<double, double> dim_extent =
        layout->getSpatialExtents(sdom_id, dim);
  ...
  index_size[IMoment::getName()] =
      (input_vars->legendre_order + 1) * (input_vars->legendre_order + 1);
  ...
  // allocate storage for the sweep boundary data (upwind and downwind share
  // same buffer)
  plane_data[0] = new SubTVec(kernel->nestingPsi(),
                              num_groups,
                              num_directions,
                              nzones[1] * nzones[2]);
  plane_data[1] = new SubTVec(kernel->nestingPsi(),
                              num_groups,
                              num_directions,
                              nzones[0] * nzones[2]);
  plane_data[2] = new SubTVec(kernel->nestingPsi(),
                              num_groups,
                              num_directions,
                              nzones[0] * nzones[1]);

  // For block-jacobi parallel method
  old_plane_data[0] = new SubTVec(kernel->nestingPsi(),
                                  num_groups,
                                  num_directions,
                                  nzones[1] * nzones[2]);
  old_plane_data[1] = new SubTVec(kernel->nestingPsi(),
                                  num_groups,
                                  num_directions,
                                  nzones[0] * nzones[2]);
  old_plane_data[2] = new SubTVec(kernel->nestingPsi(),
                                  num_groups,
                                  num_directions,
                                  nzones[0] * nzones[1]);
  ...
  psi =
      new SubTVec(kernel->nestingPsi(), num_groups, num_directions, num_zones);
  psi->clear(0.0);
  rhs =
      new SubTVec(kernel->nestingPsi(), num_groups, num_directions, num_zones);
  ...
  double sample_vol_frac =
      1.0 / (double)(num_subsamples * num_subsamples * num_subsamples);
  int zone_id = 0;
  double pz = zeros[2];
  ...
  for (int k = 0; k < nzones[2]; k++) {
    ...
    for (int j = 0; j != nzones[1]; j++) {
      ...
      for (int i = 0; i != nzones[0]; i++) {
        ...
        double zone_volume =
            deltas[0][i + 1] * deltas[1][j + 1] * deltas[2][k + 1];
...
void Subdomain::setVars(SubTVec *ell_ptr,
...
bool Subdomain::compare(Subdomain const &b, double tol, bool verbose) {
  ...
  namess << "gdset[gs=" << idx_group_set << ", ds=" << idx_dir_set
         << ", zs=" << idx_zone_set << "]";
  ...
  is_diff |=
      compareVector(name + ".deltas[0]", deltas[0], b.deltas[0], tol, verbose);
  is_diff |=
      compareVector(name + ".deltas[1]", deltas[1], b.deltas[1], tol, verbose);
  is_diff |=
      compareVector(name + ".deltas[2]", deltas[2], b.deltas[2], tol, verbose);
...
void Subdomain::computeSweepIndexSet(void) {
  ...
  if (false) {  // try a dummy sweep pattern.. this will give you the WRONG
                // answer.. just to understand memory performance
    int z = 0;
    int zstart = z;
    for (int k = 0; k < nzones[2]; ++k) {
      for (int j = 0; j < nzones[1]; ++j) {
    ...
    sweep_block.indexset_sweep.push_back(RAJA::RangeSegment(zstart, z));
  } else {
    for (int C = 0; C <= (3 * N);
         ++C) {  // for each C we can touch zone["i","j","k"]  as well as "d"
                 // and "group"    in parallel
      int FLAG = 0;
      for (int i = 0; i <= C; ++i) {
        for (int j = 0; j <= C; ++j) {
          ...
          if (ii <= i_max && jj <= j_max && kk <= k_max && ii >= i_min
              && jj >= j_min
              && kk >= k_min) {
