...
RAJA_INLINE void callKernelWithPolicy(Nesting_Order nesting_order,
                                      KERNEL kernel,
                                      ARGS &... args) {
  switch (nesting_order) {
    case NEST_DGZ:
      kernel(NEST_DGZ_T(), args...);
      break;
#ifndef RAJA_COMPILER_ICC
    case NEST_DZG:
      kernel(NEST_DZG_T(), args...);
      break;
    case NEST_GDZ:
      kernel(NEST_GDZ_T(), args...);
      break;
    case NEST_GZD:
      kernel(NEST_GZD_T(), args...);
      break;
    case NEST_ZDG:
      kernel(NEST_ZDG_T(), args...);
      break;
    case NEST_ZGD:
      kernel(NEST_ZGD_T(), args...);
      break;
#else
    default:
      KripkeAbort(
          "All nesting orders except DGZ are currently disabled with the Intel "
          "compilers\n");
#endif
  }
}
...
Kernel::Kernel(Nesting_Order nest) : nesting_order(nest) {}

Kernel::~Kernel() {}

struct Kernel_LTimes {
  template <typename nest_type>
  RAJA_INLINE void operator()(nest_type, Grid_Data &domain) const {
    ...
    dForallN<LTimesPolicy<nest_type>, IMoment, IDirection, IGroup, IZone>(
        domain, sdom_id, LTimesFcn<PHI, ELL, PSI>(phi, ell, psi, group0));
#else
    dForallN<LTimesPolicy<nest_type>, IMoment, IDirection, IGroup, IZone>(
        domain,
        sdom_id,
...
struct Kernel_Scattering {
  ...
  RAJA_INLINE void operator()(nest_type, Grid_Data &domain) const {
    ...
    typename POL::View_MixedToZones mixed_to_zones(
        domain, sdom_id, (IZone *)&sdom.mixed_to_zones[0]);
    typename POL::View_MixedToMaterial mixed_material(
        domain, sdom_id, (IMaterial *)&sdom.mixed_material[0]);
    typename POL::View_MixedToFraction mixed_fraction(domain,
                                                      sdom_id,
                                                      &sdom.mixed_fraction[0]);
    typename POL::View_MomentToCoeff moment_to_coeff(
        domain, sdom_id, (ILegendre *)&domain.moment_to_coeff[0]);

#ifdef KRIPKE_USE_FUNCTORS
    dForallN<ScatteringPolicy<nest_type>,
             IMoment,
             IGlobalGroup,
             IGlobalGroup,
             IMix>(
        ...
        ScatteringFcn<typename POL::View_Phi,
                      ...
                      typename POL::View_MomentToCoeff>(phi,
                                                        phi_out,
                                                        sigs,
                                                        mixed_to_zones,
                                                        mixed_material,
...
struct Kernel_Sweep {
  template <typename nest_type>
  RAJA_INLINE void operator()(nest_type, Grid_Data &domain, int sdom_id) const {
    typedef DataPolicy<nest_type> POL;
    ...
    typename POL::View_FaceI face_lf(domain,
                                     sdom_id,
                                     sdom->plane_data[0]->ptr());
    typename POL::View_FaceJ face_fr(domain,
                                     sdom_id,
                                     sdom->plane_data[1]->ptr());
    typename POL::View_FaceK face_bo(domain,
                                     sdom_id,
    ...
    typename POL::View_IdxToI idx_to_i(domain,
                                       sdom_id,
                                       (IZoneI *)&extent.idx_to_i[0]);
    typename POL::View_IdxToJ idx_to_j(domain,
                                       sdom_id,
                                       (IZoneJ *)&extent.idx_to_j[0]);
    typename POL::View_IdxToK idx_to_k(domain,
                                       sdom_id,
                                       (IZoneK *)&extent.idx_to_k[0]);
    ...
    RAJA::forallN<SweepPolicy<nest_type>, IDirection, IGroup, IZoneIdx>(
        ...
        SweepFcn<typename POL::View_Directions,
                 ...
                 typename POL::View_IdxToK>(direction,
                                            rhs,
                                            psi,
                                            sigt,
                                            dx,
    ...
    RAJA::forallN<SweepPolicy<nest_type>, IDirection, IGroup, IZoneIdx>(
        ...
        RAJA_LAMBDA(IDirection d, IGroup g, IZoneIdx zone_idx) {
          ...
          double const psi_d_g_z =
              (rhs(d, g, z) + face_lf(d, g, j, k) * xcos_dxi
