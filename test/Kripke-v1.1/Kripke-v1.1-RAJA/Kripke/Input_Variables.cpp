...
// make sure any output only goes to root
  ...
  if (num_zonesets_dim[0] <= 0 || num_zonesets_dim[1] <= 0
      || num_zonesets_dim[2] <= 0) {
    if (!rank)
      printf(
          "Number of zone-sets in each dim need to be greater than or equal to "
          "1\n");
    return true;
  }

  if (layout_pattern < 0 || layout_pattern > 1) {
    if (!rank) printf("Layout(%d) must be either 0 or 1\n", layout_pattern);
    return true;
  }

  if (nesting < 0) {
    if (!rank) printf("Invalid nesting selected\n");
    return true;
  }
  ...
  if (num_groups % num_groupsets) {
    if (!rank)
      printf(
          "Number of groups (%d) must be evenly divided by number of groupsets "
          "(%d)\n",
          num_groups,
          num_groupsets);
    return true;
  ...
  if (num_directions % num_dirsets) {
    if (!rank)
      printf(
          "Number of directions (%d) must be evenly divided by number of "
          "directionsets(%d)\n",
          num_directions,
          num_dirsets);
    return true;
  ...
  if (niter < 1) {
    if (!rank) printf("You must run at least one iteration (%d)\n", niter);
    return true;
  }
