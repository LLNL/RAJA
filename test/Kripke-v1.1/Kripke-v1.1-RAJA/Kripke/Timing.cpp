...
void Timing::clear(void) { timers.clear(); }
...
void Timing::print(void) const {
  ...
  for (int i = 0; i < names.size(); ++i) {
    printf("  %-16s  %12d  %12.5lf",
           names[i].c_str(),
           (int)ord_timers[i]->count,
           ord_timers[i]->total_time);
#ifdef KRIPKE_USE_PAPI
...
void Timing::setPapiEvents(std::vector<std::string> names) {
  ...
  for (int i = 0; i < names.size(); ++i) {
    ...
    int event_code;
    PAPI_event_name_to_code(const_cast<char *>(names[i].c_str()), &event_code);
    ...
    if (retval != PAPI_OK) {
      fprintf(stderr,
              "ERROR ADDING %s, retval=%d, ID=0x%-10x\n",
              names[i].c_str(),
              retval,
              event_code);
    }
