...
void usage(void) {
  ...
  if (myid == 0) {
    ...
    printf("-------------------------------\n");
    printf(
        "  --layout <lout>        Layout of spatial subdomains over mpi "
        "ranks\n");
    printf(
    ...
    printf(
        "                         Must be a factor of 8, and divide evenly the "
        "number\n");
    ...
    printf(
        "                         Must divide evenly the number energy "
        "groups\n");
    ...
    printf("                         Default:  --zset %d,%d,%d\n\n",
           def.num_zonesets_dim[0],
           def.num_zonesets_dim[1],
...
struct CmdLine {
  ...
  bool atEnd(void) { return (cur >= size); }
...
int main(int argc, char **argv) {
  ...
  if (myid == 0) {
    ...
    printf(
        ...
        "\n");
    printf(
        "This work was produced at the Lawrence Livermore National "
        "Laboratory\n");
    printf(
...
#pragma omp parallel
    ...
    printf("GroupSet/Groups:       %d sets, %d groups/set\n",
           vars.num_groupsets,
           vars.num_groups / vars.num_groupsets);
    printf("DirSets/Directions:    %d sets, %d directions/set\n",
           vars.num_dirsets,
           vars.num_directions / vars.num_dirsets);

    printf("Zone Sets:             %d,%d,%d\n",
           vars.num_zonesets_dim[0],
           vars.num_zonesets_dim[1],
           vars.num_zonesets_dim[2]);
  }
