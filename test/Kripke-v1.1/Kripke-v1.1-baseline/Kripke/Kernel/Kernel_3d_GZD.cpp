...
#pragma omp parallel for
        ...
        for (int zone = 0; zone < num_zones; ++zone) {
          ...
          for (int mix = mix_start; mix < mix_stop; ++mix) {
            ...
            double const *KRESTRICT sigs_g_gp_mat =
                sigs_g_gp + material * num_coeff;
    ...
    for (int k = extent.start_k; k != extent.end_k; k += extent.inc_k) {
      ...
      for (int j = extent.start_j; j != extent.end_j; j += extent.inc_j) {
        ...
        for (int i = extent.start_i; i != extent.end_i; i += extent.inc_i) {
          ...
          double *KRESTRICT psi_lf_g_z =
              psi_lf_g + I_PLANE_INDEX(j, k) * num_directions;
          double *KRESTRICT psi_fr_g_z =
              psi_fr_g + J_PLANE_INDEX(i, k) * num_directions;
          double *KRESTRICT psi_bo_g_z =
              psi_bo_g + K_PLANE_INDEX(i, j) * num_directions;
