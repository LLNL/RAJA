...
#endif
    if (myid == 0) {
      KripkeAbort("ERROR: Incorrect number of MPI tasks. Need %d MPI tasks.",
                  R);
    }
  }
...
/**
  Computes the subdomain ID based on a given groupset, directionset, and
  zoneset.
*/
...
BlockLayout::BlockLayout(Input_Variables *input_vars) : Layout(input_vars) {}
...
*/
std::pair<double, double> BlockLayout::getSpatialExtents(int sdom_id,
                                                         int dim) const {
  ...
  std::pair<double, double> ext_proc(ext_global.first
                                         + dx * (double)our_rank[dim],
                                     ext_global.first
                                         + dx * (double)(our_rank[dim] + 1));
  ...
  double sdx =
      (ext_proc.second - ext_proc.first) / (double)num_zone_sets_dim[dim];
  std::pair<double, double> ext_sdom(ext_proc.first + sdx * (double)zs_dim,
                                     ext_proc.first
                                         + sdx * (double)(zs_dim + 1));
...
/**
  Compute the spatial extents of a subdomain along a given dimension.
*/
std::pair<double, double> ScatterLayout::getSpatialExtents(int sdom_id,
                                                           int dim) const {
  // Start with global problem dimensions
  std::pair<double, double> ext_global(-60.0, 60.0);
  if (dim == 1) {
  ...
  // Subdivide by number of subdomains in specified dimension
  double sdx =
      (ext_global.second - ext_global.first) / (double)num_zone_sets_dim[dim];
  std::pair<double, double> ext_sdom(ext_global.first + sdx * (double)zs_dim,
                                     ext_global.first
                                         + sdx * (double)(zs_dim + 1));

  // Subdivide by number of processors in specified dimension
  double dx = (ext_sdom.second - ext_sdom.first) / (double)num_procs[dim];
  std::pair<double, double> ext_proc(ext_sdom.first
                                         + dx * (double)our_rank[dim],
                                     ext_sdom.first
                                         + dx * (double)(our_rank[dim] + 1));
