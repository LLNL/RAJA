...
// "Read" litmus test for DESUL ordered atomic
// ---------------------------------------------------
...
// Atomic policy: Relaxed (Weak), SeqCst
...
struct ReadLitmus {
  ...
  RAJA_HOST_DEVICE void run(int this_thread, int other_thread, int iter)
  {
    ...
    if (swap) {
      read_1(other_thread, iter);
      read_2(this_thread, iter);
    } else {
      read_2(this_thread, iter);
      read_1(other_thread, iter);
    }
  ...
  RAJA_HOST_DEVICE void read_1(int thread, int iter)
  {
    int thread_idx = thread * m_stride + iter;
    RAJA::atomicExchange<AtomicPolicy>(&(x[thread_idx]), T{1});
    RAJA::atomicExchange<RelaxedPolicy>(&(y[thread_idx]), T{1});
  }

  RAJA_HOST_DEVICE void read_2(int thread, int iter)
  {
    int thread_idx = thread * m_stride + iter;
    RAJA::atomicExchange<AtomicPolicy>(&(y[thread_idx]), T{2});
    a[thread_idx] = RAJA::atomicAdd<RelaxedPolicy>(&(x[thread_idx]), T{0});
  }
  ...
  void count_results(camp::resources::Resource work_res)
  {
    ...
    RAJA::forall<GPUExec>(forall_len, [=] RAJA_HOST_DEVICE(int i) {
      if (a_local[i] == 1 && y_local[i] == 2) {
        // Strong behavior: thread 1 happened before thread 2
        strong_cnt_0 += 1;
      } else if (a_local[i] == 0 && y_local[i] == 1) {
        // Strong behavior: thread 2 happened before thread 1
        strong_cnt_1 += 1;
      } else if (a_local[i] == 1 && y_local[i] == 1) {
        // Strong behavior: stores interleaved with receives
        interleaved_cnt += 1;
      } else if (a_local[i] == 0 && y_local[i] == 2) {
        // Weak behavior: receives reordered after stores
        weak_cnt += 1;
      } else {
        unexpected_cnt += 1;
      }
...
using ReadLitmusTestOrderPols =
    camp::list<RAJA::atomic_relaxed, RAJA::atomic_seq_cst>;

using ReadLitmusTestPols =
    camp::cartesian_product<AtomicDataTypeList, ReadLitmusTestOrderPols>;
...
TYPED_TEST_P(ForallAtomicLitmusTest, ReadTest)
{
  using Type = typename camp::at<TypeParam, camp::num<0>>::type;
  using AtomicPol = typename camp::at<TypeParam, camp::num<1>>::type;

  using ReadTest = ReadLitmus<Type, AtomicPol>;
  LitmusTestDriver<ReadTest>::run();
}

REGISTER_TYPED_TEST_SUITE_P(ForallAtomicLitmusTest, ReadTest);

using ReadTestTypes = Test<ReadLitmusTestPols>::Types;
