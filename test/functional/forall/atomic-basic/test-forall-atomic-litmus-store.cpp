...
// "Store" litmus test for DESUL ordered atomic
// ---------------------------------------------------
...
struct StoreLitmus {
  ...
  RAJA_HOST_DEVICE void store_1(int thread, int iter)
  {
    ...
    RAJA::atomicExchange<RelaxedPolicy>(&(x[thread_idx]), T{2});
    RAJA::atomicAdd<SendPolicy>(&(y[thread_idx]), T{1});
  }

  RAJA_HOST_DEVICE void store_2(int thread, int iter)
  {
    int thread_idx = thread * m_stride + iter;
    a[thread_idx] = RAJA::atomicAdd<RecvPolicy>(&(y[thread_idx]), T{0});
...
using StoreLitmusTestOrderPols =
...
using StoreLitmusTestPols =
    camp::cartesian_product<AtomicDataTypeList, StoreLitmusTestOrderPols>;
...
TYPED_TEST_P(ForallAtomicLitmusTest, StoreTest)
{
  ...
  using StoreTest = StoreLitmus<Type, SendPol, RecvPol>;
  LitmusTestDriver<StoreTest>::run();
}

REGISTER_TYPED_TEST_SUITE_P(ForallAtomicLitmusTest, StoreTest);

using StoreTestTypes = Test<StoreLitmusTestPols>::Types;
