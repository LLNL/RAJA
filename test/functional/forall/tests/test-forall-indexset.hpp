...
// RDH  iset.push_back(LIST_TYPE(&lseg[0], lseg_len));
...
// RDH  iset.push_back(LIST_TYPE(lseg_vec));
...
// RDH  iset.push_back(ListSegType(&lseg[0], lseg_len));
  iset.push_back(ListSegType(&lseg[0], lseg_len, &working_res));
  last_idx = lseg[lseg_len - 1];
...
// RDH  iset.push_back(ListSegType(lseg_vec));
  iset.push_back(ListSegType(lseg_vec, &working_res));
  last_idx = lseg_vec[lseg_len - 1];
...
// RDH  iset.push_back(ListSegType(lseg_vec));
  ...
  size_t num_idx = is_indices.size();

#else

  IndexSetType iset; 
  buildIndexSet<INDEX_TYPE, 
                RangeSegType, RangeStrideSegType, ListSegType>(iset, 
                                                               &working_res);
  ...
  size_t num_idx = is_indices.size();
  INDEX_TYPE last_idx = is_indices[num_idx - 1];

#endif  // MOVED TO FUNCTION
  ...
  //  
//  RDH  Resource working_res{WORKING_RES()};
