...
struct KernelNestedLoopTest<DEPTH_2_COLLAPSE, WORKING_RES, EXEC_POLICY>{
  template <typename... ExtraArgs>
  KernelNestedLoopTest(){
    KernelNestedLoopTest<DEPTH_2, WORKING_RES, EXEC_POLICY>();
  }
};
...
struct MultiLambdaNestedLoopExec { using type = void; };
...
TYPED_TEST_P(KernelNestedLoopMultiLambdaTest, NestedLoopMultiLambdaKernel) {
  ...
  using EXEC_POLICY = typename MultiLambdaNestedLoopExec<LOOP_DEPTH, EXEC_POL_DATA>::type;

  // For double nested loop tests the third arg is ignored.
  KernelNestedLoopTest<LOOP_DEPTH, WORKING_RES, EXEC_POLICY>();
  KernelNestedLoopTest<LOOP_DEPTH, WORKING_RES, EXEC_POLICY>();
}
