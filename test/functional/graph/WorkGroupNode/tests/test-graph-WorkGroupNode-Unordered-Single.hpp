...
///
/// Header file containing tests for RAJA WorGroupNode unordered runs.
///

#ifndef __TEST_GRAPH_WORKGROUPNODE_UNORDERED_SINGLE__
#define __TEST_GRAPH_WORKGROUPNODE_UNORDERED_SINGLE__
...
template <typename GraphPolicy,
          ...
          >
void testWorkGroupNodeUnorderedSingle(IndexType begin, IndexType end)
{
...
template <typename T>
class WorkGroupNodeBasicUnorderedSingleFunctionalTest : public ::testing::Test
{
};

TYPED_TEST_SUITE_P(WorkGroupNodeBasicUnorderedSingleFunctionalTest);


TYPED_TEST_P(WorkGroupNodeBasicUnorderedSingleFunctionalTest, BasicWorkGroupNodeUnorderedSingle)
{
  ...
  testWorkGroupNodeUnorderedSingle< GRAPH_POLICY, ExecPolicy, OrderPolicy, StoragePolicy, IndexType, Allocator, WORKING_RESOURCE >(b1, e1);
  testWorkGroupNodeUnorderedSingle< GRAPH_POLICY, ExecPolicy, OrderPolicy, StoragePolicy, IndexType, Allocator, WORKING_RESOURCE >(b2, e2);
  testWorkGroupNodeUnorderedSingle< GRAPH_POLICY, ExecPolicy, OrderPolicy, StoragePolicy, IndexType, Allocator, WORKING_RESOURCE >(b3, e3);
}

#endif  //__TEST_GRAPH_WORKGROUPNODE_UNORDERED_SINGLE__
