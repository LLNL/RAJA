...
static void PrintCommandLineOptions(char *execname, int myRank) {
  if (myRank == 0) {
    ...
    printf(
        " -b <balance>    : Load balance between regions of a domain (def: "
        "1)\n");
    printf(
        " -c <cost>       : Extra cost of more expensive regions (def: 1)\n");
    printf(
        " -f <numfiles>   : Number of files to split viz dump into (def: "
        "(np+10)/9)\n");
    printf(" -p              : Print out progress\n");
    printf(
        " -v              : Output viz file (requires compiling with "
        "-DVIZ_MESH\n");
  ...
  if (argc > 1) {
    ...
    while (i < argc) {
      ...
      if (strcmp(argv[i], "-i") == 0) {
        ...
        if (!ok) {
          ParseError(
              "Parse Error on option -i integer value required after "
              "argument\n",
              myRank);
        }
        i += 2;
      ...
      /* -s <size, sidelength> */
        ...
        if (!ok) {
          ParseError(
              "Parse Error on option -s integer value required after "
              "argument\n",
              myRank);
        }
        i += 2;
      ...
      /* -r <numregions> */
        ...
        if (!ok) {
          ParseError(
              "Parse Error on option -r integer value required after "
              "argument\n",
              myRank);
        }
        i += 2;
      ...
      /* -f <numfilepieces> */
        ...
        if (!ok) {
          ParseError(
              "Parse Error on option -f integer value required after "
              "argument\n",
              myRank);
        }
        i += 2;
      ...
      /* -q */
        ...
        if (!ok) {
          ParseError(
              "Parse Error on option -b integer value required after "
              "argument\n",
              myRank);
        }
        i += 2;
        ...
        if (!ok) {
          ParseError(
              "Parse Error on option -c integer value required after "
              "argument\n",
              myRank);
        }
        i += 2;
...
void VerifyAndWriteFinalOutput(Real_t elapsed_time,
  ...
  for (Index_t j = 0; j < nx; ++j) {
    for (Index_t k = j + 1; k < nx; ++k) {
      Real_t AbsDiff =
          fabs(locDom.e(iperm[j * nx + k]) - locDom.e(iperm[k * nx + j]));
