...
public:
  ...
  inline T&  operator()( integer_t index0, integer_t index1 )
  {
    return operatorParen( index0, index1, m_data, m_lengths );
//    return m_data[index0*stride<2>(m_lengths)+index1];
  }
  inline T const & operator()( integer_t index0, integer_t index1 ) const
  {
    return operatorParen( index0, index1, m_data, m_lengths );
//    return m_data[index0*stride<2>(m_lengths)+index1];
  }

  static inline T &  operatorParen( integer_t index0,
                                    integer_t index1,
                                    T * restrict data,
                                    integer_t const * restrict lengths)
  {
    return data[index0*stride<2>(lengths)+index1];
  }
