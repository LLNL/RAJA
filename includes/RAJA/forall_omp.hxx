...
#pragma omp parallel for 
      for ( int i = 0; i < nthreads; ++i ) {
         m_min[i*COHERENCE_BLOCK_SIZE/sizeof(T)] = init_val ;
      }
   }
   ...
   operator T()
   {
      if ( !m_reduction_is_final ) {
         int nthreads = omp_get_max_threads();
         for ( int i = 0; i < nthreads; ++i ) {
            m_reduced_val = RAJA_MIN(m_reduced_val, static_cast<T>(m_min[i*COHERENCE_BLOCK_SIZE/sizeof(T)]));
         }
   ...
   ReduceMin<omp_reduce, T> min(T val) const 
   {
      ...
      m_min[tid] = RAJA_MIN(static_cast<T>(m_min[tid*COHERENCE_BLOCK_SIZE/sizeof(T)]), val);
      return *this ;
   }
...
#pragma omp parallel for 
      for ( int i = 0; i < nthreads; ++i ) {
         m_sum[i*COHERENCE_BLOCK_SIZE/sizeof(T)] = 0 ;
      }
   }
   ...
   operator T()
   {
      if ( !m_reduction_is_final ) {
         int nthreads = omp_get_max_threads();
         for ( int i = 0; i < nthreads; ++i ) {
            m_reduced_val += static_cast<T>(m_sum[i*COHERENCE_BLOCK_SIZE/sizeof(T)]);
         }
